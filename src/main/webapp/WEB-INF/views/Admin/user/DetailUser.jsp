			...
			<main>
				<div class="container-fluid">
					...
					<div class="card mb-4">
						...
						<div class="row">
							<div class="col-sm-4">Mã Khách Hàng:  ${uses.id }</div>
							<div class="col-sm-8">Họ Tên khách hàng: ${uses.lastName }</div>
						</div>
						<div class="row">
							<div class="col-sm-4">Số điện thoại:  ${uses.numberphone }</div>
							<div class="col-sm-8">Địa chỉ: ${uses.address }</div>
						</div>
						<div class="row">
							<div class="col-sm-4">Trạng thái tài khoản: 
								<c:if test="${uses.status eq 'true'}">
									<span class="badge badge-success">Bình Thường</span>
								</c:if>
								<c:if test="${uses.status eq 'false'}">
									<span class="badge badge-danger">Khóa</span>
								</c:if>
							</div>
							<div class="col-sm-8">Loại Tài Khoản: ${roles.description }</div>
						</div>
						<div class="row">
							<div class="col-sm-4">Số lần mua hàng:  ${count }</div>
							<div class="col-sm-8">Loại hàng yêu thích: </div>
						</div>
